...
local function Hop()
    -- Gửi thông báo chờ hop
    pcall(function()
        require(game:GetService("ReplicatedStorage").Notification).new("<Color=Red>Lonely Hub:<Color=/> Wait ".. tostring(getgenv().TimeHop) .."s hop server"):Display()
    end)
    
    task.wait(TimeHopServer or getgenv().TimeHop or 3)
    ...
    pcall(function()
        ...
        for i = 1, 100 do
            ...
            if ok and typeof(res) == "table" then
                for jobId, data in pairs(res) do
                    ...
                    -- Tìm server có Bounty cao nhất, dưới 12 người và KHÔNG PHẢI server hiện tại
                    if bounty > best.Bounty and players < 12 and jobId ~= game.JobId then
                        best.JobId = jobId
                        best.Bounty = bounty
        ...
        -- Teleport vào server tốt nhất vừa tìm được
        if best.JobId then
